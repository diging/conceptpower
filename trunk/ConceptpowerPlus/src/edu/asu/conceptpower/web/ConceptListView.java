...
public class ConceptListView extends DatabaseBean implements Serializable {
	...
	public void init() {
		...
		try {
			conceptManager = new ConceptManager(manager, controller
		...
		if (conceptList != null)
			founds = conceptManager.getConceptListEntries(conceptList
					.getConceptListName());
		else if (FacesUtil.getSessionMapValue(Parameter.SELECTED_LIST) != null) {
			Object selectedListObj = FacesUtil
					.getSessionMapValue(Parameter.SELECTED_LIST);

			if (selectedListObj instanceof ConceptList)
				founds = conceptManager
						.getConceptListEntries(((ConceptList) selectedListObj)
								.getConceptListName());
			else {
				founds = conceptManager
						.getConceptListEntries((String) selectedListObj);
			}
		}

		ConceptTypesManager typeManager;
		try {
			typeManager = new ConceptTypesManager(controller
		...
		DatabaseManager userDBManager = controller.getDatabaseProvider()
		...
		entries = wrapperCreator
				.createWrappers(
						founds != null ? founds.toArray(new ConceptEntry[founds
								.size()]) : new ConceptEntry[0], usersManager,
						typeManager, conceptManager);
	...
	public void setSelectedConceptForShow(
			ConceptEntryWrapper selectedConceptForShow) {
	...
	public ListDataModel<ConceptEntryWrapper> getDatamodel() {
		...
		((ListDataModel<ConceptEntryWrapper>) datamodel)
				.setWrappedData(entries);
		return datamodel;
	}
