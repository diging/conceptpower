...
public class DatabaseClient {
	...
	public DatabaseClient(ObjectContainer wordnetCacheClient, ObjectContainer dictionaryClient) {
		this.wordnetCacheClient = wordnetCacheClient;
	...
	public ConceptEntry getEntry(String id) {
		 ...
		 */
		results = wordnetCacheClient.queryByExample(exampleEntry);
	...
	public ConceptEntry[] getEntriesByFieldContains(String field, String containsString) {
		...
		ObjectSet<ConceptEntry> results = dictionaryClient.query(new Predicate<ConceptEntry>() {
					public boolean match(ConceptEntry con) {
						...
						// go through all fields of class to find field that should be searched
						Field[] fields = con.getClass().getDeclaredFields();
						for (Field field : fields) {
							SearchField searchFieldAnnotation = field.getAnnotation(SearchField.class);
							if (searchFieldAnnotation != null) {
								// if we found the field that should be search through
...
//		}
	...
	public void closeConnection() {
		wordnetCacheClient.close();
		dictionaryClient.close();
	}
}
